...
class SPSRModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        self.fake_H_branch, self.fake_H, self.grad_LR = self.netG(self.var_L)# 生成的梯度图 + 预测值 + 低分辨率的梯度

        
        self.fake_H_grad = self.get_grad(self.fake_H) #预测结果的梯度
        self.var_H_grad = self.get_grad(self.var_H) #真实影像的梯度
        self.var_ref_grad = self.get_grad(self.var_ref) #同上
    ...
    def testforrs(self,var_Lrs):
        self.netG.eval()
        with torch.no_grad():
            self.fake_H_branch, self.fake_H, self.grad_LR = self.netG(var_Lrs)

        self.netG.train()
        return self.fake_H_branch, self.fake_H
